...
const CitizenRequests = () => {
  ...
  const [isAIProcessingEnabled, setIsAIProcessingEnabled] = useState(false); // Автоматическая обработка отключена по умолчанию
  const [isAISettingsOpen, setIsAISettingsOpen] = useState(false);
  const [aiProcessingSettings, setAiProcessingSettings] = useState({
    useOrgStructure: false,   // Использование правил орг. структуры
    useAgentRules: false,    // Использование правил AI агентов
    autoClassify: false,     // Автоматическая классификация отключена
    autoRespond: false       // Автоматические ответы отключены
  });
  ...
  const [processingRequests, setProcessingRequests] = useState<Record<number, boolean>>({});
  ...
  // Функция для обработки запроса с помощью AI агента
  const processRequestWithAgent = async (request: CitizenRequest, agentId: number, actionType: string) => {
    // Добавляем запрос в список обрабатываемых
    ...
    try {
      ...
      const response = await fetch(`/api/citizen-requests/${request.id}/process-with-agent`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Ошибка при обработке запроса агентом');
      }
      
      const result = await response.json();
      
      // Обновляем статус запроса в интерфейсе
      ...
      // Удаляем запрос из списка обрабатываемых
      setProcessingRequests(prev => {
        const newState = {...prev};
      ...
      console.error('Ошибка при обработке с агентом:', error);
      
      // Удаляем запрос из списка обрабатываемых даже при ошибке
      setProcessingRequests(prev => {
        const newState = {...prev};
        delete newState[request.id];
        return newState;
      ...
      toast({
        title: "Ошибка",
        description: `Не удалось обработать запрос: ${error instanceof Error ? error.message : 'Неизвестная ошибка'}`,
        variant: "destructive"
      });
