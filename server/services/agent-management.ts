...
import { AIAnalytics } from '../monitoring/ai-analytics';
...
export class AgentManagementService {
  ...
  async processRequest(requestData: any) {
    ...
    try {
      ...
      const result = await this.repository.processAgentRequest(requestData);
      
      // Track metrics
      this.analytics.trackRequest(Date.now() - startTime, true);
      
      // Create blockchain record
      await this.createBlockchainRecord(result);
      
      return result;
    } catch (error) {
      this.analytics.trackRequest(Date.now() - startTime, false);
      throw error;
    }
  }

  private async createBlockchainRecord(data: any) {
    const record = {
      timestamp: Date.now(),
      data,
      hash: ''
    };
    record.hash = await this.blockchain.calculateHash(record);
    return record;
  }
}
